...
// send Output: Out EventDataPort Data_Types__Temp_i
Unit t_AttestationGate_Filter_Impl_Filter_Filter_seL4Nix_Output_Send(
  STACK_FRAME
  art_DataContent d) {
  DeclNewStackFrame(caller, "sb_Filter_Impl.c", "", "t_AttestationGate_Filter_Impl_Filter_Filter_seL4Nix_Output_Send", 0);

  sb_Output_enqueue(d);
}

// is_empty Input: In EventDataPort
B t_AttestationGate_Filter_Impl_Filter_Filter_seL4Nix_Input_IsEmpty(STACK_FRAME_ONLY) {
  return sb_Input_is_empty();
}

// receive Input: In EventDataPort union_art_DataContent
Unit t_AttestationGate_Filter_Impl_Filter_Filter_seL4Nix_Input_Receive(
  STACK_FRAME
  Option_8E9F45 result) {
  DeclNewStackFrame(caller, "sb_Filter_Impl.c", "", "t_AttestationGate_Filter_Impl_Filter_Filter_seL4Nix_Input_Receive", 0);
...
void pre_init(void) {
  ...
  t_AttestationGate_Filter_Impl_Filter_Filter_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  t_AttestationGate_Filter_Impl_Filter_Filter_adapter_initialiseEntryPoint(SF_LAST);
...
int run(void) {
  ...
  for(;;) {
    ...
    t_AttestationGate_Filter_Impl_Filter_Filter_adapter_computeEntryPoint(SF_LAST);
    sb_self_pacer_tick_emit();
  }
  return 0;
}
