...
B t_AttestationGate_Receiver_i_preceiver_receiver_seL4Nix_read_port_IsEmpty(STACK_FRAME_ONLY) {
  return sb_read_port_is_empty();
}

// receive read_port: In EventDataPort union_art_DataContent
Unit t_AttestationGate_Receiver_i_preceiver_receiver_seL4Nix_read_port_Receive(
  STACK_FRAME
  Option_8E9F45 result) {
  DeclNewStackFrame(caller, "sb_Receiver_i.c", "", "t_AttestationGate_Receiver_i_preceiver_receiver_seL4Nix_read_port_Receive", 0);
...
void pre_init(void) {
  DeclNewStackFrame(NULL, "sb_Receiver_i.c", "", "pre_init", 0);

  printf("Entering pre-init of Receiver_i_preceiver_receiver\n");
  ...
  t_AttestationGate_Receiver_i_preceiver_receiver_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  t_AttestationGate_Receiver_i_preceiver_receiver_adapter_initialiseEntryPoint(SF_LAST);

  printf("Leaving pre-init of Receiver_i_preceiver_receiver\n");
}

void post_init(void) {
  DeclNewStackFrame(NULL, "sb_Receiver_i.c", "", "post_init", 0);
 ...
 ************************************************************************/
int run(void) {
  DeclNewStackFrame(NULL, "sb_Receiver_i.c", "", "run", 0);

  MUTEXOP(sb_dispatch_sem_wait())
  for(;;) {
    MUTEXOP(sb_dispatch_sem_wait())
    // call the component's compute entrypoint
    t_AttestationGate_Receiver_i_preceiver_receiver_adapter_computeEntryPoint(SF_LAST);
  }
  return 0;
}
